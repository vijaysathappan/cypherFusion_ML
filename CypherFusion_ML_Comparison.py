...
def main():
    file_path = "D:\\Downloads\\NetflixOriginals.csv"
    ...
    X_train, X_test, y_train, y_test = split_data(X_numeric, y_numeric)
    ...
    model.fit(X_train.values, y_train, use_xgboost=False)  # Note: y_train is now numeric
    evaluate_model(model, X_test.values, y_test)
    ...
    model_xgboost.fit(X_train.values, y_train, use_xgboost=True)  # Note: y_train is now numeric
    evaluate_model(model_xgboost, X_test.values, y_test)
